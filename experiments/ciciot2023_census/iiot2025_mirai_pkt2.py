...
FLOODS=sorted(p for p in glob.glob(EX+"/malware/*mirai*.pcap") if "/._" not in p)  # syn+udp Mirai floods
...
vic_mac={}
for p in FLOODS:
    name=re.sub(r".*mirai-[a-z]+-flood_|--.*|\.pcap$","",os.path.basename(p))  # target device (merges syn+udp)
...
json.dump({"n_victim_devices":len(devices),"attack_flows":len(X),"benign_flows":len(ben_rows),
          ...
          open(os.path.join(HERE,"iiot2025_mirai_pkt2.json"),"w"),indent=2)
print("Wrote iiot2025_mirai_pkt2.json")
